// forked from design-sync lib/css.mjs - drop @font-face rules whose local url()s all fail to resolve
...
export function extractFonts(cssPath, srcDir, { fontsOut, roots }) {
  ...
  for (const m of css.matchAll(/@font-face\s*\{([^}]+)\}/g)) {
    ...
    let rewritten = body;
    // A face is only shippable if at least one of its local url()s resolved to
    // a file we actually copied into fonts/. Root-absolute urls (a host app's
    // /assets/x.woff2) resolve against the filesystem root and never exist, so
    // without this the rule ships pointing at a file that isn't uploaded -
    // the same dead-src hazard rewriteBundleFontFaces guards against below.
    let localUrls = 0;
    let resolved = 0;
    for (const u of urls) {
    ...
    if (localUrls && !resolved) {
      console.error(`  ! @font-face dropped: ${fam} - no local url() resolved (${urls.join(', ')})`);
      continue;
    }
